...
describe('User controller', function() {
  ...
  it('Create a user', function(done) {
    request(sails.hooks.http.app).post('/user/create').send(user).expect(200).end(function(err, response) {
      if (err) {
        return done(err);
      }
...
describe('Image controller', function() {
  it('Create an image', function(done) {
    request(sails.hooks.http.app).post('/image/create').send({
      owner: user.id,
      url: 'http://google.com'
...
describe('Associations', function() {
  it('User images', function(done) {
    request(sails.hooks.http.app).get('/user').expect(200).end(function(err, response) {
